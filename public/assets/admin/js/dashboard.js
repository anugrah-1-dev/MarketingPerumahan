...
function renderTopAgents(agents) {
    ...
    if (!agents || agents.length === 0) {
        container.innerHTML =
            '<p style="color:#94a3b8;text-align:center;padding:1.5rem">Belum ada data agent.</p>';
        return;
    }
...
function renderRecentActivity(activities) {
    ...
    if (!activities || activities.length === 0) {
        tbody.innerHTML =
            '<tr><td colspan="4" style="text-align:center;color:#94a3b8;padding:1.5rem">Belum ada aktivitas.</td></tr>';
        return;
    }
    const badgeMap = {
